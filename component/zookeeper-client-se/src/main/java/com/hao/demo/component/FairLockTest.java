...
public class FairLockTest {
    ...
    private void ensureRootPath() {
        try {
            if (zk.exists(lockName, true) == null) {
                zk.create(lockName, "".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE,
    ...
    public void lock() {
        ...
        try {
            ...
            if (!path.isEmpty() && !minPath.get(0).isEmpty() && path.equals(
                lockName + "/" + minPath.get(0))) {
            ...
            if (watchNode != null) {
                ...
                if (stat != null) {
                    System.out.println(
                        "Thread " + Thread.currentThread().getId() + " waiting for " + lockName
                            + "/" + watchNode);
                }
            }
            try {
